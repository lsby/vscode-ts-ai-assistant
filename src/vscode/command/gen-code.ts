...
export async function 计算优化代码片段提示词(
  ...
  if (函数节点) {
    ...
    const 函数范围 = 函数节点.获得节点范围()

    var 函数头到选择头 = 开始位置 - 函数范围.start
    var 选择长度 = 结束位置 - 开始位置
    var 选择尾 = 函数头到选择头 + 选择长度
    定义 = `${定义.slice(0, 函数头到选择头)}<|从这里开始|>${定义.slice(函数头到选择头, 函数头到选择头 + 选择长度)}<|到这里结束|>${定义.slice(选择尾)}`
  ...
  if (类节点) {
    ...
    var 选择长度 = 结束位置 - 开始位置
    var 选择尾 = 类头到选择头 + 选择长度
    定义 = `${定义.slice(0, 类头到选择头)}<|从这里开始|>${定义.slice(类头到选择头, 类头到选择头 + 选择长度)}<|到这里结束|>${定义.slice(选择尾)}`

    var 提示词 = [
      '在typescript中, 我有一个类.',
      `对于其中我标记的部分, ${要求 ? `请: ${要求}` : '请帮我优化它'}.`,
