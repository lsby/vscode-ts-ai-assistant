...
import { 路径在node_modules里 } from './tools'
import { 范围 } from './type'

export class 程序 {
  static 创建程序(tsconfig路径: string, types路径: string): 程序 {
    var tsconfig内容 = ts.readConfigFile(tsconfig路径, ts.sys.readFile)
    var tsconfig解析结果 = ts.parseJsonConfigFileContent(tsconfig内容.config, ts.sys, path.dirname(tsconfig路径))
    return new 程序(
  ...
  按名称查找函数节点(名称: string): 函数节点 | null {
    var self = this
    var 源文件集合 = this.程序.getSourceFiles()

    for (var 源文件 of 源文件集合) {
      var 节点 = ts.forEachChild(源文件, function 查找函数节点(node): 函数节点 | null {
  ...
  按名称查找类节点(名称: string): 类节点 | null {
    ...
    for (var 源文件 of 源文件集合) {
      var 节点 = ts.forEachChild(源文件, function 查找类节点(node): 类节点 | null {
  ...
  按范围查找节点(文件路径: string, 范围: 范围): 节点 | null {
    ...
    for (var 源文件 of 源文件集合) {
      ...
      var 节点结果 = ts.forEachChild(源文件, function 查找节点(node): 节点 | null {
  ...
  获得文件引入信息(文件路径: string): { 路径: string; 名称: string }[] {
    var 引入数组: { 路径: string; 名称: string }[] = []

    var 源文件 = this.程序.getSourceFiles().find((a) => path.normalize(a.fileName) == path.normalize(文件路径))
    if (!源文件) return []

    var 引入声明 = 源文件.statements.filter(ts.isImportDeclaration)

    for (var 引入声明项 of 引入声明) {
      var 引入模块名称 = 引入声明项.moduleSpecifier.getText()
      var 引入符号 = this.类型检查器.getSymbolAtLocation(引入声明项.moduleSpecifier)
      if (引入符号 && 引入符号.declarations && 引入符号.declarations[0]) {
        var 引入声明信息 = this.类型检查器.getTypeOfSymbolAtLocation(引入符号, 引入符号.declarations[0])
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
        var 引入位置 = 引入声明信息?.symbol.getDeclarations()?.[0]?.getSourceFile().fileName
        if (引入位置) {
